...
SYSTEM_PROMPTS = {
    "analyze": (
        ...
        "- CRITICAL — for npm peer dependency conflicts: ALWAYS fix the version number "
        "in package.json (e.g. upgrade the mismatched package version). "
        "NEVER use --legacy-peer-deps or any CI workaround flag as the fix. "
        "NEVER set affected_file to a CI/workflow YAML file for a dependency version error. "
        "The error log shows 'Found: X' and 'peer requires: Y' — use those to set "
        "search_string=X and replacement_string=Y in package.json.\n"
